document.addEventListener('DOMContentLoaded', () => {
    ...
    function drawTile(connections, x, y, connectedSides) {
        ...
        ctx.lineWidth = 4; // Use thinner lines
        ctx.lineCap = 'round';
        const diamondSize = 12; // Adjust diamond size
    ...
    function drawBoard() {
        ...
        // First loop: draw all background cells and grid lines
        ...
        // Second loop: draw all tiles to ensure correct connection colors
        for (let r = 0; r < boardSize.height; r++) {
            for (let c = 0; c < boardSize.width; c++) {
                ...
                if (tileConns && !(isDragging && originalGridPos.r === r && originalGridPos.c === c)) {
                    ...
                    if (tileConns[0] === 1) { if (r > 0 && boardState[r-1][c] && boardState[r-1][c][2] === 1) connectedSides[0] = true; }
                    if (tileConns[1] === 1) { if (c < boardSize.width - 1 && boardState[r][c+1] && boardState[r][c+1][3] === 1) connectedSides[1] = true; }
                    if (tileConns[2] === 1) { if (r < boardSize.height - 1 && boardState[r+1][c] && boardState[r+1][c][0] === 1) connectedSides[2] = true; }
                    if (tileConns[3] === 1) { if (c > 0 && boardState[r][c-1] && boardState[r][c-1][1] === 1) connectedSides[3] = true; }
    ...
    async function runSolveAnimation(swaps) {
        isAnimating = true; 
        infoPanel.textContent = 'Playing solution...';
        ...
        infoPanel.textContent = 'Solve complete!'; 
        isAnimating = false; 
        drawBoard();
    }
    
    async function fetchNewPuzzle(width = 5, height = 5) {
        infoPanel.textContent = 'Generating new puzzle...'; 
        isAnimating = true;
        try {
            ...
            infoPanel.textContent = 'Drag the tiles to connect the lines!'; 
            drawBoard();
        } catch (error) {
            console.error("Failed to fetch puzzle:", error);
            infoPanel.textContent = 'Loading failed, please refresh and try again!';
        } finally {
            isAnimating = false;
        }
    }
    ...
    retryBtn.addEventListener('click', () => {
        if (initialBoardState && !isAnimating) {
            ...
            infoPanel.textContent = 'Retried, please continue!';
            drawBoard();
        }
    });
    ...
    solveBtn.addEventListener('click', async () => {
        ...
        infoPanel.textContent = 'Requesting solution from server...';
        const response = await fetch('/api/solve', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ current_grid: boardState, solution_grid: solutionState })
        });
        const data = await response.json();
        if (data.swaps && data.swaps.length > 0) {
            await runSolveAnimation(data.swaps);
        } else {
            infoPanel.textContent = 'This is already the correct answer!';
        }
    });
    ...
    imageUploader.addEventListener('change', async (event) => {
        ...
        infoPanel.textContent = 'Uploading and recognizing image...';
        isAnimating = true;
        ...
        try {
            ...
            canvas.width = boardSize.width * CELL_SIZE; 
            canvas.height = boardSize.height * CELL_SIZE;
            infoPanel.textContent = 'Image recognized, start the game!'; 
            drawBoard();
        } catch (error) {
            console.error("Image upload failed:", error);
            infoPanel.textContent = `Error: ${error.message}`;
        } finally {
            isAnimating = false;
            imageUploader.value = '';
        }
